...
## 6. 收工：正確關閉 Raspberry Pi（嚴禁直接拔電）

Pi 的整個系統都在 microSD 卡上。直接斷電若正好碰上寫入，輕則檔案毀損、重則整張卡的
檔案系統壞掉、下次開不了機。**收工一律先軟關機、再斷電：**
...
- 執行後 SSH 會立刻斷線，這是正常現象，不是當機。
- **等約 20 秒**、Pi 板上綠色活動燈不再閃爍後，才可以拔電源。
- 想驗證真的關了：從地面電腦確認 SSH 連不上即可（`ssh pi@<IP>` 應該連線逾時）。
- 下次開機：插回電源就會自動開機（Pi 沒有電源鍵）。
- 只是要重開機（不斷電）：`sudo reboot`。

---

## 7. 本次實測驗收紀錄（2026-07-20）
